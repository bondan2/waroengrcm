...
export default function Login() {
    ...
    if (result.success) {
      ...
      setTimeout(() => {
        if (role === 'admin') {
          ...
          // Customer -> ke dashboard customer
  ...
  const handleGoogleLogin = async () => {
    ...
    // Google login akan redirect otomatis via OAuth
  }
        ...
        <div className="relative">
          ...
          <div className="text-center mb-6 sm:mb-8">
            <motion.div
              ...
              className="w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br from-orange-500 to-red-600 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-orange-500/25"
            >
          ...
          <div className="flex bg-gray-100 rounded-xl p-1 mb-6">
            ...
            <button onClick={() => setLoginMethod('google')}
              className={`flex-1 flex items-center justify-center space-x-2 py-2.5 rounded-lg text-sm font-medium transition-all ${
                loginMethod === 'google' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500'
              }`}>
              <span>Google</span>
            </button>
          </div>
          ...
          {/* Email Login Form */}
          {loginMethod === 'email' && (
            <motion.form initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? (
                  <><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div><span>Memproses...</span></>
                ) : (
          ...
          <div className="mt-6 text-center space-y-3">
            <p className="text-sm text-gray-500">
              Belum punya akun?{' '}
              <Link to="/register" className="text-orange-600 font-medium hover:underline">Daftar sekarang</Link>
            </p>
